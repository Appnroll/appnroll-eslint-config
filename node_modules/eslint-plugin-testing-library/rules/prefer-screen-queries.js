    ...
    create: function (context) {
        ...
        return _a = {
                VariableDeclarator: function (node) {
                    var isWithinFunction = node_utils_1.isCallExpression(node.init) && node_utils_1.isIdentifier(node.init.callee) && node.init.callee.name === 'within';
                    if (!isWithinFunction) {
                        return;
                    }
                    if (node_utils_1.isObjectPattern(node.id)) {
                        var identifiers = node.id.properties
                            .filter(function (property) { return node_utils_1.isProperty(property) && node_utils_1.isIdentifier(property.key) && queriesRegex.test(property.key.name); })
                            .map(function (property) { return property.key.name; });
